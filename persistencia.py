...
class Persistencia:
    ...
    def cargar_datos(self):
        ...
        for dato_libro in datos['libros']:
          libro = LibroFisico(titulo=dato_libro['titulo'], autor=dato_libro['autor'], isbn=dato_libro['isbn'], disponible=dato_libro['disponible'])
          biblioteca.libros.append(libro)

        for dato_usuario in datos['usuarios']:
          if 'carrera' in dato_usuario:
            usuario = Estudiante(nombre=dato_usuario['nombre'], cedula=dato_usuario['cedula'], carrera=dato_usuario['carrera'])
          else:
            usuario = Profesor(nombre=dato_usuario['nombre'], cedula=dato_usuario['cedula'])
